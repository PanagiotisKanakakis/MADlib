...
SUPPORTED_PORTS = ('postgres', 'greenplum', 'monetdb')
...
def _check_db_port(portid):
    ...
    try:
        if portid == 'postgres':
            ...
            row = _internal_run_query("select value from sys.environment where name = 'monet_version'", True)
    except:
...
def _plpy_check(py_min_ver):
    ...
    if portid != 'monetdb':
        ...
        info_(this, "Testing Python environment...", True)
        rv = _internal_run_query("select count(*) as cnt from function_languages where language_name = 'Python2'", True)
        if int(rv[0]['cnt']) > 0:
            info_(this, "> Python already installed", verbose)
        else:
            error_(this, "> PL/Python not installed", verbose)
            raise Exception
...
def _is_madlib_installation_valid_for_tests(schema, db_madlib_ver, test_type):
    ...
    if not db_madlib_ver:
        info_(this, "MADlib is not installed in the schema {0}. {1} stopped.".format(schema, test_type.capitalize()),
              True)
        return False
...
def main(argv):
    ...
    if portid:
        ...
        if portid in SUPPORTED_PORTS:
            if portid == 'monetdb':
                # replace with correct env vars
                if c_user is None:
                    c_user = os.environ.get('PGUSER', 'monetdb')
                if c_pass is None:
                    c_pass = os.environ.get('PGPASSWORD', 'monetdb')
                if c_host is None:
                    c_host = os.environ.get('PGHOST', 'localhost')
                if c_port is None:
                    c_port = os.environ.get('PGPORT', '50000')
                if c_db is None:
                    c_db = os.environ.get('PGDATABASE', getpass.getuser())
            else:
                if c_user is None:
        ...
        global con_args
        con_args['portid'] = portid
        con_args['host'] = c_host + ':' + c_port
        con_args['database'] = c_db
        con_args['user'] = c_user
        if portid != 'monetdb':
            con_args['cmd'] = 'psql'
            con_args['password'] = 'postgres'
        else:
            con_args['cmd'] = 'mclient'
            con_args['password'] = 'monetdb'
        ...
        if dbver is None:
            ...
            if portid == 'greenplum':
                ...
                if is_rev_gte(dbver_split, get_rev_num('11.3')):
                    dbver = str(dbver_split[0])
    ...
    if args.command[0] in ('install-check', 'dev-check'):
        ...
        try:
            if portid == 'postgres':
                is_schema_in_db = _internal_run_query(
                    "SELECT schema_name FROM information_schema.schemata WHERE schema_name='%s';" % schema, True)
            else:
                is_schema_in_db = _internal_run_query(
                    "SELECT  name from sys.schemas where name ='%s';" % schema, True)
        ...
        if return_val == 0:
            ...
            if result == 'FAIL':
                ...
                if args.command[0] != 'uninstall':
                    if args.command[0] == 'upgrade':
                        info_(this, "MADlib %s upgraded successfully in %s schema." % (str(new_madlib_ver), schema),
                              True)
                    else:
