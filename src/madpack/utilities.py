...
def get_db_madlib_version(con_args, schema):
    ...
    """
    try:
        if con_args['portid'] == 'postgres' or con_args['portid'] == 'greenpulm':
            ...
            n_madlib_versions = int(run_query(
                """
                    select count(*) as cnt 
                    from sys.schemas ss, sys.tables st 
                    where ss.id = st.schema_id and ss.name = '{0}' and st.name = 'migrationhistory'
                """.format(schema), con_args, True)[0]['cnt'])
...
def get_dbver(con_args, portid):
    ...
    try:
        ...
        if portid == 'monetdb':
            versionStr = subprocess.check_output('mclient -v', shell=True)
            match = re.search("version[a-zA-Z\s]*(\d+\.\d+)", versionStr)
        else:
