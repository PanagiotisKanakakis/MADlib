...
#  include("${CMAKE_CURRENT_LIST_DIR}/FindMonetDB.cmake")
#  -->8--
#
...
#
# This package locates MT_config and uses it to determine all other paths. If
# ${PKG_NAME}_MT_CONFIG is defined, then the search steps will be omitted.
#
...
set(_NEEDED_MT_CONFIG_PACKAGE_NAME "MonetDB")
set(_MT_CONFIG_VERSION_NUM_MACRO "PACKAGE_STRING")
set(_MT_CONFIG_VERSION_MACRO "PACKAGE_VERSION")
...
# If ${PKG_NAME}_MT_CONFIG is defined, then the search steps will be omitted.
if(NOT DEFINED ${PKG_NAME}_MT_CONFIG)
    find_program(${PKG_NAME}_MT_CONFIG monetdb
            HINTS ${_SEARCH_PATH_HINTS}
            )
endif(NOT DEFINED ${PKG_NAME}_MT_CONFIG)

if(${PKG_NAME}_MT_CONFIG)
    find_program(${PKG_NAME}_SERVER_INCLUDE_DIR monetdb_config.h
            HINTS ${_SEARCH_PATH_HINTS}
            )
    set(${PKG_NAME}_SERVER_INCLUDE_DIR  "/usr/include/monetdb")
    set(${PKG_NAME}_CLIENT_INCLUDE_DIR  "/usr/include/monetdb")
endif(${PKG_NAME}_MT_CONFIG)

if(${PKG_NAME}_MT_CONFIG AND ${PKG_NAME}_SERVER_INCLUDE_DIR)
    set(${PKG_NAME}_VERSION_MAJOR 0)
    set(${PKG_NAME}_VERSION_MINOR 0)
    set(${PKG_NAME}_VERSION_PATCH 0)

    set(CONFIG_FILE ${${PKG_NAME}_SERVER_INCLUDE_DIR}/monetdb_config.h)
    if(EXISTS ${CONFIG_FILE})
        ...
        if(${CMAKE_COMPILER_IS_GNUCC})
            ...
            execute_process(
                    ...
                    OUTPUT_VARIABLE _MT_CONFIG_HEADER_CONTENTS
            )
        else(${CMAKE_COMPILER_IS_GNUCC})
            file(READ ${CONFIG_FILE} _MT_CONFIG_HEADER_CONTENTS)
        endif(${CMAKE_COMPILER_IS_GNUCC})

        # Get PACKAGE_NAME
        if (_MT_CONFIG_HEADER_CONTENTS MATCHES "#define PACKAGE_NAME \".*\"")
            string(REGEX REPLACE
                    ".*#define PACKAGE_NAME \"([^\"]+)\".*" "\\1"
                    _PACKAGE_NAME "${_MT_CONFIG_HEADER_CONTENTS}")
        else(_MT_CONFIG_HEADER_CONTENTS MATCHES "#define PACKAGE_NAME \".*\"")
            message(FATAL_ERROR "Unable to locate PACKAGE_NAME in \"${CONFIG_FILE}\".")
        endif(_MT_CONFIG_HEADER_CONTENTS MATCHES "#define PACKAGE_NAME \".*\"")

        # Get VERSION
        string(REGEX REPLACE
                ".*#define PACKAGE_STRING \"([^\"]+)\".*" "\\1"
                _MT_VERSION "${_MT_CONFIG_HEADER_CONTENTS}")

        if (_MT_VERSION MATCHES "${_NEEDED_MT_CONFIG_PACKAGE_NAME} ([0-9]+.*)")
            string(REGEX REPLACE
                    "${_NEEDED_MT_CONFIG_PACKAGE_NAME} ([0-9]+.*)" "\\1"
                    ${PKG_NAME}_VERSION_NUM "${_MT_VERSION}")
        else(_MT_VERSION MATCHES "${_NEEDED_MT_CONFIG_PACKAGE_NAME} ([0-9]+.*)")
            set(${PKG_NAME}_VERSION_NUM "unknown")
        endif(_MT_VERSION MATCHES "${_NEEDED_MT_CONFIG_PACKAGE_NAME} ([0-9]+.*)")
        ...
        if(${PKG_NAME}_VERSION_NUM MATCHES "^[0-9]+$")
            ...
            if (_MT_VERSION MATCHES "${_NEEDED_MT_CONFIG_PACKAGE_NAME} ([0-9]+.*)")
                string(REGEX REPLACE
                        "${_NEEDED_MT_CONFIG_PACKAGE_NAME} ([0-9]+).*" "\\1"
                        ${PKG_NAME}_VERSION_STRING "${_MT_VERSION}")

                string(REGEX REPLACE "${_NEEDED_MT_CONFIG_PACKAGE_NAME} ([0-9]+).*" "\\1"
                        ${PKG_NAME}_VERSION_MAJOR "${_MT_VERSION}")

                string(REGEX REPLACE "${_NEEDED_MT_CONFIG_PACKAGE_NAME} [0-9]+\\.([0-9]+).*" "\\1"
                        ${PKG_NAME}_VERSION_MINOR "${_MT_VERSION}")
        ...
        message(FATAL_ERROR "Found mt_config (\"${${PKG_NAME}_MT_CONFIG}\"), "
                "but monetdb_config.h file not present in the "
                "server include dir (${${PKG_NAME}_SERVER_INCLUDE_DIR}).")
    endif(EXISTS ${CONFIG_FILE})

endif(${PKG_NAME}_MT_CONFIG AND ${PKG_NAME}_SERVER_INCLUDE_DIR)
